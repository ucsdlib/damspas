...
feature 'Visitor wants to create/edit a language' do
  ...
  after do
	...
	scenario 'is on mads index page' do
		sign_in_developer
		visit "mads_languages"
		expect(page).to have_selector('a', :text => "Create Language")

		# Change to create button
		visit "mads_languages/new"
		# Create new language
		fill_in "Name", :with => "TestLabel"
		fill_in "Code", :with => "TestCode"
		...
		# Save path of language for other test(s)
    ...
    # the name gets overwritten by what is in the "Element Value", so this test no longer makes sense.
		...
		expect(page).to have_selector('li', :text => "TestCode")
	...
	scenario 'is on the language page to be edited' do
		sign_in_developer

		visit Path.path
		click_on "Edit"
		fill_in "Name", :with => "Edited Test Language"
		fill_in "Code", :with => "TestCode"
		...
		#expect(page).to have_selector('strong', :text => "Edited Test Language")
	end
...
feature 'Visitor wants to cancel unsaved edits' do
	...
	scenario 'is on Edit Language page' do
		sign_in_developer
		visit Path.path
		expect(page).to have_selector('a', :text => "Edit")
		click_on "Edit"
		...
		#expect(page).to have_content("Edited Test Language")
	end
