...
feature 'Visitor wants to see the collection record' do
  ...
  scenario 'should see the names in order' do
    ...
    expect(page).to have_selector("div.span8 dl dt[9]", :text => 'Principal Investigator')
    expect(page).to have_selector("div.span8 dl dt[11]", :text => 'Co Principal Investigator')
    expect(page).to have_selector("div.span8 dl dt[13]", :text => 'Laboratory Director')
    expect(page).to have_selector("div.span8 dl dt[15]", :text => 'Thesis Advisor')
    expect(page).to have_selector("div.span8 dl dt[17]", :text => 'Contributors')
  end
